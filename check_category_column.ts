...
async function check() {
  const { data, error } = await supabase.from('participants').select('category').limit(1)
  if (error) {
    console.log('Category column exists? NO, error:', error.message)
  } else {
    console.log('Category column exists? YES')
  }
}
