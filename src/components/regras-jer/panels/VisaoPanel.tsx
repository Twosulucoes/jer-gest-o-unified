  ...
  const modalidades = useMemo(() => {
    ...
    return MODALIDADES.filter((m) => {
      ...
      return (
        ...
        m.slug.includes(f) ||
        m.confederacao.toLowerCase().includes(f)
      );
    });
  }, [filtro, bloco]);
    ...
    <Tabs defaultValue="modalidades" className="w-full">
      ...
      <TabsContent value="modalidades" className="space-y-4">
        <div className="flex flex-wrap gap-2 items-center">
          ...
          <Badge variant="secondary">
            {modalidades.length} de {MODALIDADES.length}
          </Badge>
        </div>
      ...
      <TabsContent value="categorias" className="space-y-4">
        <RegraSection title="JER's — Categorias padrão">
          <KvTable
            items={CATEGORIAS_PADRAO.jers.map((c) => ({
              label: c.categoria,
              value: `${c.anos} · ${c.generos}`,
            }))}
          />
        </RegraSection>
        <RegraSection title="JERPA — Categorias padrão">
          <KvTable
            items={CATEGORIAS_PADRAO.jerpa.map((c) => ({
              label: c.categoria,
              value: `${c.anos} · ${c.generos}`,
            }))}
          />
        ...
        <RegraSection title="Categorias especiais por modalidade">
          <KvTable
            items={CATEGORIAS_PADRAO.especiais.map((c) => ({
              label: `${c.modalidade} · ${c.categoria}`,
              value: c.anos,
            }))}
          />
        </RegraSection>
      ...
      <TabsContent value="pontuacao" className="space-y-4">
        <RegraSection title="Tabela de pontos do Campeonato Geral (Art. 108)">
          <KvTable
            items={PONTUACAO_GERAL.map((p) => ({ label: p.colocacao, value: `${p.pontos} pts` }))}
          />
        </RegraSection>
        ...
        <RegraSection title="Modalidades sem correspondência nacional">
          <KvTable
            items={SEM_NACIONAL.map((s) => ({ label: s.categoria, value: s.modalidades }))}
          />
        </RegraSection>
        <RegraSection title="Seleção Estadual — Coletivas 15–17" description={SELECAO_ESTADUAL.intro}>
          <KvTable
            items={SELECAO_ESTADUAL.linhas.map((l) => ({
              label: l.total,
              value: `Campeã JER's: ${l.campea} · Seletiva ampla: ${l.seletiva}`,
            }))}
          />
          <ul className="text-xs text-muted-foreground list-disc list-inside mt-2">
            {SELECAO_ESTADUAL.notas.map((n, i) => (
              <li key={i}>{n}</li>
            ))}
          </ul>
        </RegraSection>
      ...
      <TabsContent value="calendario" className="space-y-4">
        <RegraSection title="JER's — Fase Final (Boa Vista)">
          <KvTable
            items={CALENDARIO.jers.map((g) => ({
              label: `Grupo ${g.grupo} · ${g.execucao}`,
              value: g.modalidades,
            }))}
          />
        </RegraSection>
