...
import { Swords, RefreshCw, ExternalLink, AlertTriangle } from "lucide-react";
...
export default function CentralMatchesTab({ eventId, sportEventId, isCollective, onChanged }: Props) {
  ...
  const handleOpenGenDialog = async () => {
    setSelectedPhaseId(phases[0]?.id ?? "");
    setBlockingReason(null);

    // Check if regeneration is safe when matches exist
    if (matches.length > 0) {
      const { data, error } = await supabase.rpc("rpc_can_regenerate_matches", {
        p_event_id: eventId,
        p_sport_event_id: sportEventId,
      });
      if (!error && data) {
        const result = data as any;
        if (!result.can_regenerate) {
  ...
  const generateMut = useMutation({
    ...
    onSuccess: (data: any) => {
      const skipped = data.entries_skipped ?? data.teams_skipped ?? 0;
      const msg = skipped > 0
        ? `${data.matches_created} partida(s) gerada(s). ${skipped} entrada(s) pulada(s) por inelegibilidade.`
        : `${data.matches_created} partida(s) gerada(s).`;
      toast({ title: msg, variant: skipped > 0 ? "default" : "default" });
      setShowGenDialog(false);
      qc.invalidateQueries({ queryKey: ["central-matches"] });
      qc.invalidateQueries({ queryKey: ["eligibility-pending"] });
      onChanged();
    },
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Button size="sm" onClick={handleOpenGenDialog} disabled={phases.length === 0}>
          ...
          {blockingReason ? (
            ...
            <div className="space-y-3">
              ...
              <p className="text-xs text-muted-foreground">
                Partidas com status "scheduled" serão removidas e recriadas. Atletas/equipes inelegíveis serão pulados automaticamente.
              </p>
            </div>
          )}
