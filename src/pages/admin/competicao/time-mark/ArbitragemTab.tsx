...
const ROLES = [
  ...
  { id: "arbitro_auxiliar", label: "Árbitro Auxiliar" },
  { id: "cronometrista", label: "Cronometrista" },
  { id: "juiz_chegada", label: "Juiz de Chegada" },
  { id: "juiz_partida", label: "Juiz de Partida" },
...
export function ArbitragemTab({ sportEventId }: ArbitragemTabProps) {
  ...
  const [selectedRole, setSelectedRole] = useState<string>("arbitro_principal");

  // Fetch sport_id to filter referees
  const { data: eventInfo } = useQuery({
    queryKey: ["sport_event_info", sportEventId],
    queryFn: async () => {
  ...
  // Note: For now, we'll use match_user_assignments and aggregate at sport_event level
  const { data: assignments = [], isLoading } = useQuery({
    queryKey: ["arbitragem_assignments", sportEventId],
    queryFn: async () => {
      ...
      (data as any[]).forEach(a => {
        ...
        if (!unique.has(key)) {
          unique.set(key, {
            id: a.id, // Just one of the match assignment IDs
            userId: a.user_id,
            role: a.role,
  ...
  // Fetch available referees (profiles with specific roles or linked to the sport)
  const { data: availableReferees = [] } = useQuery({
    queryKey: ["available_referees", eventInfo?.sport_id],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("profiles")
        .select("id, full_name")
        .limit(100); // Should be filtered by sport_id in a real scenario
      if (error) throw error;
      return data;
    },
  ...
  const addAssignmentMutation = useMutation({
    mutationFn: async ({ userId, role }: { userId: string, role: string }) => {
      ...
      if (!matches || matches.length === 0) {
        throw new Error("Não há séries ou provas montadas para atribuir árbitros.");
      }
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["arbitragem_assignments", sportEventId] });
  ...
  return (
    <div className="space-y-6">
        ...
        <Dialog open={showAddModal} onOpenChange={setShowAddModal}>
          ...
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                O árbitro selecionado será atribuído a todas as séries e fases desta prova.
              </DialogDescription>
            </DialogHeader>
      ...
      <div className="bg-muted/30 border rounded-lg p-3 flex gap-3 text-xs text-muted-foreground italic">
        ...
        <p>A equipe designada aqui é replicada para todas as séries (heats/finais) da prova selecionada.</p>
      </div>
