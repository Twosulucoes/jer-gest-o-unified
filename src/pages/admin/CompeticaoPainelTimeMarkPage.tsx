...
import { Timer, Users, ListChecks, Calendar, Scale, Search } from "lucide-react";
...
import { InscritosTab } from "./competicao/time-mark/InscritosTab";
import { SeriesTab } from "./competicao/time-mark/SeriesTab";
import { AgendaTab } from "./competicao/time-mark/AgendaTab";
import { ArbitragemTab } from "./competicao/time-mark/ArbitragemTab";

export default function CompeticaoPainelTimeMarkPage() {
  ...
  const { data: sportEvents = [], isLoading: loadingEvents } = useQuery({
    queryKey: ["sport_events_time_mark", sportId, stageId],
    enabled: !!sportId,
    queryFn: async () => {
      const { data, error } = await supabase
  ...
  const filteredEvents = useMemo(() => {
    return sportEvents.filter((ev: any) => {
      ...
      // Status filter placeholder - can be implemented based on actual data
  ...
  return (
    <div className="space-y-6">
      <ModuleHeader 
        route={headerRoute}
        title={`Painel Time/Mark — ${currentSportEvent?.sports?.name || "Modalidade"}`}
      />
      ...
      <div className="grid grid-cols-1 gap-6">
        ...
        <div className="bg-muted/30 p-4 rounded-lg border space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-semibold uppercase tracking-wider text-muted-foreground">Provas</h3>
            </div>
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="bg-muted border-b">
                  ...
                  <th className="px-3 py-2 text-center font-medium">Séries</th>
                ...
                {filteredEvents.map((ev: any) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-3 py-2 text-center">{ev.phase_count || 0}</td>
                      <td className="px-3 py-2">
                        <Badge variant="outline" className="text-[10px] py-0 h-4">
                          {Number(ev.phase_count) > 0 ? "Montado" : "Não montado"}
                        </Badge>
                      </td>
        ...
        {selectedEventId ? (
            ...
            <Tabs defaultValue="inscritos" className="w-full">
              <TabsList className="grid grid-cols-4 w-full max-w-xl">
                ...
                <TabsTrigger value="series" className="gap-2">
                  <ListChecks className="h-4 w-4" /> Séries
                </TabsTrigger>
              ...
              <div className="mt-6">
                ...
                <TabsContent value="series" className="m-0">
                  <SeriesTab sportEventId={selectedEventId} />
                </TabsContent>
          ...
          <div className="flex flex-col items-center justify-center py-20 border-2 border-dashed rounded-xl text-muted-foreground">
            <Timer className="h-12 w-12 opacity-20 mb-4" />
