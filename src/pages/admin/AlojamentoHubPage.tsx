...
import {
  Plus, Pencil, Building, DoorOpen, KeyRound, AlertTriangle, ArrowRight,
} from "lucide-react";
...
export default function AlojamentoHubPage() {
  ...
  const { data: locations = [], isLoading: loadingLocations } = useQuery({
    ...
    queryFn: async () => {
      const q = supabase.from("lodging_locations").select("*").order("name");
  ...
  const { data: occupancyCounts = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        ...
        .eq("event_id", selectedEventId).in("status", ["allocated", "checked_in"]);
      if (error) throw error;
      return data;
    },
    enabled: !!selectedEventId,
  ...
  const createUnit = useMutation({
    mutationFn: async (v: LodgingUnitFormValues) => {
      const { error } = await supabase.from("lodging_units").insert({
        event_id: selectedEventId, location_id: v.location_id, name: v.name,
        capacity: v.capacity, gender_restriction: v.gender_restriction,
  ...
  const canAdd = tab === "locais" ? events.length > 0 : (selectedEventId && locations.length > 0);
    ...
    <div className="animate-fade-in space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Configure locais e unidades de hospedagem</p>
        </div>
        {canWrite && (
      ...
      <Tabs value={tab} onValueChange={(v) => setTab(v as TabKey)}>
        ...
        <TabsContent value="unidades" className="mt-4">
          {!selectedEventId ? (
            <EmptyState icon={<DoorOpen className="h-10 w-10" />} message="Selecione um evento no seletor acima" />
          ) : locations.length === 0 ? (
            <div className="rounded-lg border border-amber-200 bg-amber-50 dark:bg-amber-950/20 dark:border-amber-900 p-4 flex items-start gap-3">
              ...
              <div className="space-y-1">
                <p className="text-sm font-medium text-amber-800 dark:text-amber-300">Nenhum local cadastrado ainda</p>
                <p className="text-xs text-amber-700 dark:text-amber-400">Cadastre o local de hospedagem (ex: Hotel Central) antes de criar os quartos/unidades.</p>
                <Button size="sm" variant="outline" className="mt-2 gap-1 border-amber-300 text-amber-800 hover:bg-amber-100 dark:text-amber-300 dark:border-amber-700"
                  onClick={() => { setEditingLocation(null); setLocationDialog(true); setTab("locais"); }}>
                  Cadastrar local agora <ArrowRight className="h-3.5 w-3.5" />
                </Button>
              </div>
            </div>
          ) : loadingUnits ? (
            ...
            <EmptyState icon={<DoorOpen className="h-10 w-10" />} message="Nenhuma unidade cadastrada">
        ...
        <TabsContent value="locais" className="mt-4">
          {loadingLocations ? <LoadingSkeleton /> : locations.length === 0 ? (
            <EmptyState icon={<Building className="h-10 w-10" />} message="Nenhum local cadastrado">
                  ...
                  {locations.map((l) => {
                    ...
                    return (
                      <TableRow key={l.id}>
                        ...
                        <TableCell>
                          <Badge variant="outline">{unitCount} unidade{unitCount !== 1 ? "s" : ""}</Badge>
                        </TableCell>
