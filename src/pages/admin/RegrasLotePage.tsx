...
import { CoverageStats } from "@/components/regras-jer/batch/CoverageStats";
import { BatchActionButtons } from "@/components/regras-jer/batch/BatchActionButtons";
...
export default function RegrasLotePage() {
  ...
  const {
    ...
    queryFn: async () => {
      const { data: sportEvents, error: seErr } = await supabase
        .from("sport_events")
        .select(
          "id, name, slug, sport_id, category_id, is_active, sports!inner(name, is_collective), categories!inner(name, gender_scope)",
        )
        .eq("event_id", eventId!)
        .eq("is_active", true)
        .order("name");
  ...
  const seedMutation = useMutation({
    ...
    onSuccess: (report) => {
      ...
      if (!report.dry_run) {
        ...
        toast.success(
          `Regras geradas: ${report.created} criadas, ${report.updated} atualizadas, ${report.skipped} ignoradas`,
        );
      } else {
  ...
  const sportNames = useMemo(() => {
    ...
    rows.forEach((r) => {
      if (r.sport_name) set.add(r.sport_name);
    });
  ...
  const filtered = useMemo(() => {
    return rows.filter((r) => {
      if (
        ...
        !r.sport_name.toLowerCase().includes(search.toLowerCase())
      )
        return false;
  ...
  const usedPresets = useMemo(() => {
    const set = new Set<string>();
    rows.forEach((r) => {
      if (r.preset_key) set.add(r.preset_key);
    });
    return Array.from(set).sort();
  }, [rows]);
  ...
  function getOriginBadge(row: SportEventRow) {
    ...
    return (
      <Badge variant="outline" className="text-[9px] border-green-400 text-green-600">
    ...
    return (
      <div className="space-y-6">
        ...
        <Alert variant="destructive">
          <AlertDescription>
            Acesso negado. Perfil sem permissão para visualizar regras.
          </AlertDescription>
        </Alert>
      </div>
    );
  ...
  return (
    <div className="space-y-6">
      ...
      <BatchActionButtons
        ...
        totalProvas={stats.total}
        onSeed={(mode, dryRun) => seedMutation.mutate({ mode, dryRun })}
      />
      ...
      {lastReport && (
        <Alert>
          <AlertDescription className="text-xs">
            <strong>{lastReport.dry_run ? "Simulação" : "Resultado"}:</strong> {lastReport.created}{" "}
            criadas, {lastReport.updated} atualizadas, {lastReport.skipped} ignoradas.
            {" — "}Presets:{" "}
            {Object.entries(lastReport.by_preset)
              .map(([k, v]) => `${k}: ${v}`)
              .join(", ")}
          </AlertDescription>
        </Alert>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <Select value={filterJerpa} onValueChange={setFilterJerpa}>
          <SelectTrigger className="w-32">
            <SelectValue placeholder="JER/JERPA" />
          </SelectTrigger>
          <SelectContent>
        ...
        <Select value={filterCategory} onValueChange={setFilterCategory}>
          <SelectTrigger className="w-44">
            <SelectValue placeholder="Tipo de preset" />
          </SelectTrigger>
          <SelectContent>
              ...
              {filtered.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={10} className="text-center text-muted-foreground py-8">
                    {rows.length === 0
                      ? "Nenhuma prova cadastrada para este evento."
                ...
                filtered.map((r) => (
                  <TableRow key={r.id}>
                    <TableCell className="font-medium">{r.sport_name}</TableCell>
                    ...
                    <TableCell className="text-xs uppercase">{r.family ?? "—"}</TableCell>
                    <TableCell className="text-xs">{r.format ?? "—"}</TableCell>
                    <TableCell className="text-center text-xs">{r.minimo_participantes}</TableCell>
                    <TableCell>{getOriginBadge(r)}</TableCell>
