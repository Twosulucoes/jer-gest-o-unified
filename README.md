...
>
> **Voucher 🟡 Auditoria 2026-04-27** — Estado atual do módulo de voucher diagnosticado e divergências em relação à regra de negócio reformulada (voucher exclusivo para pessoas eventuais sem credencial e amarrado a serviço específico) registradas em [`docs/modulos/voucher-auditoria.md`](docs/modulos/voucher-auditoria.md). Principais lacunas: ausência de cadastro de pessoa eventual, voucher sem vínculo a instância de serviço, falta de proteção contra duplo consumo na mesma janela, ausência de fluxo formal de reemissão e de `batch_id` para lote. Implementação será feita em etapas posteriores.
