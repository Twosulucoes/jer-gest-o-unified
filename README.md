...
> **Interfaces Administrativas — Lote, Elegibilidade e OSC ✅** — Telas de alocação em lote para alojamento (wizard), gestão de direitos por janela de refeição (regras por delegação/perfil), variação de modelo de credencial por tipo de participante e interface completa para gestão de evidências operacionais OSC (2026-04-26)
...
> **Pessoas Unificadas + Vouchers QR ✅** — Toda pessoa do evento (atletas, técnicos, mesários, motoristas, cozinheiras, fiscais, terceiros) é cadastrada como **Pessoa única** (CPF como chave), podendo ter **múltiplas funções**. Vouchers QR reformulados (JER-VOU-01) liberam consumo logístico exclusivo para pessoas eventuais (sem credencial) para serviços específicos. Relatório consolidado por pessoa em `/admin/relatorios` e merge de duplicidades em `/admin/pessoas/duplicidades` (2026-04-19)
>
> **Transporte 🚌 Auditoria de Diagnóstico (2026-04-28)** — Módulo robusto para embarque QR e manifestos nominais. Identificada lacuna em **desembarque operacional no PWA** e necessidade de **alocação em lote** no administrativo. Detalhes em [`docs/modulos/transporte-auditoria.md`](docs/modulos/transporte-auditoria.md).
> **Alojamento 🏠 UNIFICADO & VALIDADO ✅** — Fase 1 de unificação estrutural concluída e confirmada via mini-auditoria. Planejamento (Admin) e execução (PWA) operam sobre fonte única de verdade no esquema `public`. Rastreabilidade total via `lodging_audit_logs` e registro de divergência funcional (2026-04-28). Detalhes em [`docs/modulos/alojamento-auditoria.md`](docs/modulos/alojamento-auditoria.md).
> **Alimentação 🍽️ FECHADO ✅** — Módulo operacional completo. Inclui gestão de janelas com automação robusta via **Padrões de Janelas** editáveis e duplicação entre estágios, elegibilidade por perfil/delegação, PWA offline funcional e **Previsão de Demanda** com exportação auditável para cozinha (2026-04-28). Detalhes em [`docs/modulos/alimentacao-auditoria.md`](docs/modulos/alimentacao-auditoria.md).
> **Reformulação Voucher 🚀 Mini-auditoria de Confirmação (2026-04-28)** — Módulo **FECHADO** e validado com evidência objetiva. Bloqueantes de schema, lógica temporal de validação e lacunas no PWA Transporte foram integralmente sanados. Auditoria consolidada em [`docs/modulos/voucher-auditoria.md`](docs/modulos/voucher-auditoria.md).
> **Módulo de Árbitros 📋 Auditoria de Fechamento ✅** — Módulo declarado **FECHADO** e pronto para operação. Cadastro técnico, designação com conflitos, integração logística, apuração automática e agenda PWA plenamente funcionais. Detalhes em [`docs/modulos/arbitros-auditoria.md`](docs/modulos/arbitros-auditoria.md). (2026-04-28)
12: > **Tipos de Participantes ✅ Delegação vs Organização** — Separação clara entre participantes de delegações (Atleta/Staff) e participantes da organização (Coordenação/Apoio), com campos administrativos específicos e regras rígidas de privacidade e exclusividade de CPF (2026-04-21)
...
| **Competição — Resultados e Governança** (Lançamento dedicado Score/Sets/Combat/Time-Mark, Placar por Período, Pênaltis, Sets, Métodos Combat, Tentativas Múltiplas com persistência, Tempo/Distância, W.O. automático, Homologação com Senha Real via Edge Function, Central de Publicação Individual/Lote, Validação KO obrigatória, Propagação automática de vencedor em Combat, Propagação manual em "serpentina" em Time/Mark com tratamento de empate) | ✅ Pronto | 100% |
| **Competição — Classificação** (Standings coletivas, Ranking individual/cross-heat, Consolidação de fase em Time/Mark, Cálculo de rank com salto após empate 1-1-3) | ✅ Pronto | 100% |
