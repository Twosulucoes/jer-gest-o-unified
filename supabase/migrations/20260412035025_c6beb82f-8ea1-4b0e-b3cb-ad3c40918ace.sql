...
DECLARE
  ...
  _cur CURSOR FOR
    SELECT se.id, se.name, se.sport_id,
           ...
           COALESCE(ser.rules->'scoring'->'walkover_policy', '{"home_goals":3,"away_goals":0}'::jsonb) as wo_policy_json,
           s.is_collective
...
BEGIN
  ...
  OPEN _cur;
  LOOP
    FETCH _cur INTO _se_id, _se_name, _se_sport_id, _family, _wo_policy_json, _se_is_collective;
    EXIT WHEN NOT FOUND;
    _cur_count := _cur_count + 1;
    _demo_se_ids := array_append(_demo_se_ids, _se_id);

    -- Extract WO scores from JSON policy
    _wo_home := COALESCE((_wo_policy_json->>'home_goals')::int, (_wo_policy_json->>'home_points')::int, 3);
    _wo_away := COALESCE((_wo_policy_json->>'away_goals')::int, (_wo_policy_json->>'away_points')::int, 0);
    ...
    FOR _i IN 1..3 LOOP
      FOR _j IN (_i+1)..4 LOOP
        ...
        IF _is_wo THEN
          _score_home := _wo_home;
          _score_away := _wo_away;
    ...
    _notes := array_append(_notes, format('SE %s (%s): 1 phase, 2 groups, 8 teams', _se_name, _family));
  END LOOP;
  CLOSE _cur;
