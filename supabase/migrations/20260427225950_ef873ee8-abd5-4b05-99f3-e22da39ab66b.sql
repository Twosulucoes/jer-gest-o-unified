...
BEGIN
    ...
    IF v_voucher.status <> 'active' THEN
        ...
        IF v_voucher.status = 'revoked' THEN
            v_metadata := v_metadata || jsonb_build_object(
                'revocation_reason', v_voucher.revocation_reason
            );
        END IF;
    ...
    v_already_used_here := false;
    IF v_voucher.voucher_type = 'nominal' THEN
        SELECT u.used_at, p.display_name INTO v_prev_usage
        ...
        WHERE u.voucher_id = v_voucher.id 
          ...
          AND (
            (p_service_kind = 'meals' AND u.meal_window_id = p_context_id) OR
            (p_service_kind = 'transport' AND u.trip_id = p_context_id) OR
            (p_service_kind = 'lodging' AND u.facility_id = p_context_id)
          )
        LIMIT 1;
        ...
        IF FOUND THEN
            v_metadata := jsonb_build_object(
                ...
                'operator_name', v_prev_usage.display_name
            );
    ...
    -- 6. Efetivar consumo
    INSERT INTO public.service_voucher_uses (
        voucher_id, 
        service_kind, 
        meal_window_id, 
        trip_id, 
        ...
        v_voucher.id, 
        p_service_kind, 
        CASE WHEN p_service_kind = 'meals' THEN p_context_id ELSE NULL END,
        CASE WHEN p_service_kind = 'transport' THEN p_context_id ELSE NULL END,
        CASE WHEN p_service_kind = 'lodging' THEN p_context_id ELSE NULL END,
        v_caller,
        v_reference_time -- Registra com o tempo real da leitura
    );
