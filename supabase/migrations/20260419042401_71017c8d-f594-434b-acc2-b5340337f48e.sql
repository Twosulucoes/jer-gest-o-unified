...
BEGIN
  ...
  -- A tabela teams não tem coluna is_active; usamos status para excluir equipes inativas/canceladas.
  SELECT COUNT(*) INTO v_current
  FROM teams t
  WHERE t.sport_event_id = NEW.sport_event_id
    AND t.delegation_id = NEW.delegation_id
    AND (TG_OP = 'INSERT' OR t.id <> NEW.id)
    AND COALESCE(t.status, 'active') NOT IN ('inactive','cancelled','canceled','removed');
