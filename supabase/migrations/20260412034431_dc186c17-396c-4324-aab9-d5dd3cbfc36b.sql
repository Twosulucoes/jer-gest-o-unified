...
BEGIN
  ...
  -- B) Delegations (8) - use 'confirmed' instead of 'active'
  FOR _i IN 1..8 LOOP
  ...
  LOOP
    ...
    -- Teams (8 per sport_event, 4 per group)
    ...
    -- Round-robin matches for Group A (teams 1-4)
    FOR _i IN 1..3 LOOP
      FOR _j IN (_i+1)..4 LOOP
        ...
        IF _is_wo THEN
          ...
          _score_away := 0;
          _outcome_home := 'wo_win';
          _outcome_away := 'wo_loss';
        ELSE
          _score_home := floor(random()*5)::int;
          _score_away := floor(random()*5)::int;
          IF _score_home > _score_away THEN
            _outcome_home := 'win'; _outcome_away := 'loss';
          ELSIF _score_home < _score_away THEN
            _outcome_home := 'loss'; _outcome_away := 'win';
          ELSE
            _outcome_home := 'draw'; _outcome_away := 'draw';
          END IF;
        END IF;
    ...
    -- Round-robin matches for Group B (teams 5-8)
    FOR _i IN 5..7 LOOP
  ...
  RETURN jsonb_build_object(
    ...
    'created', jsonb_build_object(
      'institutions', _inst_count,
      'delegations', _del_count,
      'people', _people_count,
      'participants', _part_count,
      'teams', _team_count,
      'phases', _phase_count,
      'groups', _group_count,
