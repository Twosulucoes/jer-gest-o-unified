...
-- Totals grouped by service_date × stage.
-- ============================================================
...
-- Totals grouped by meal window (date × stage × window).
-- ============================================================
...
JOIN public.meal_types mt ON mt.id = mw.meal_type_id
LEFT JOIN public.meal_consumptions mc ON mc.meal_window_id = mw.id
LEFT JOIN public.event_stages es ON es.id = mw.event_stage_id
GROUP BY
...
-- Totals grouped by operator × service_date × window.
-- ============================================================
...
-- These are the records that warrant manual review.
-- ============================================================
...
LEFT JOIN public.event_stages  es     ON es.id     = mw.event_stage_id
LEFT JOIN public.profiles      prof   ON prof.id   = mc.registered_by
WHERE mc.is_offline = true
   OR mc.duplicidade_detectada = true;
