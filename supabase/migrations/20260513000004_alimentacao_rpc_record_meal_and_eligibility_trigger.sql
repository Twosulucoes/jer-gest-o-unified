...
BEGIN
  ...
  -- 5. INSERT idempotente — sintaxe correta para partial unique index
  -- uq_meal_consumptions_active_window é CREATE UNIQUE INDEX ... WHERE status='active'
  -- portanto usa-se ON CONFLICT (cols) WHERE predicate, não ON CONSTRAINT name
