...
BEGIN
  ...
  ELSE
    -- Filtro por etapa (apenas tabelas com seed_tag)
    SELECT array_agg(pse.id) INTO v_pse_ids
      FROM participant_sport_events pse
      JOIN participants p ON p.id = pse.participant_id
    ...
    -- sport_events e categories não têm seed_tag → preservados ao filtrar por etapa
    v_se_ids := NULL;
    v_cat_ids := NULL;
  END IF;
  ...
  -- Logs e pendências (sempre por evento — não há stage_id nessas tabelas)
  IF p_stage_id IS NULL THEN
