...
DECLARE
  ...
  v_pos int := 0;
BEGIN
  INSERT INTO public.competition_matches (event_id, phase_id, sport_event_id, match_number, status, match_date)
  VALUES (v_event_id, v_phase_id, v_sport_event_id, 1, 'finished', CURRENT_DATE)
  ...
  LOOP
    ...
    INSERT INTO public.competition_match_entries (match_id, participant_sport_event_id, side)
    VALUES (v_match_id, v_pse.pse_id, 'participant')
    RETURNING id INTO v_entry_id;

    INSERT INTO public.competition_match_results (
      match_id, match_entry_id, position, points, outcome,
      time_ms, result_status, recorded_by, recorded_at,
      validated_at, published_at, published_bulletin_id
    )
    VALUES (
      v_match_id, v_entry_id, v_pos, (7 - v_pos),
      CASE WHEN v_pos = 1 THEN 'win' ELSE NULL END,
      11000 + (v_pos * 230),
      'published',
      '00000000-0000-0000-0000-000000000000', now(),
      now(), now(), v_bulletin_id
    );
  END LOOP;
END $$;
