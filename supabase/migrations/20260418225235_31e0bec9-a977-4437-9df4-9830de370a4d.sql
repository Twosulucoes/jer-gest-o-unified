CREATE OR REPLACE FUNCTION public.seed_logistics_stage_template(p_event_id uuid, p_stage_name text DEFAULT 'Etapa Modelo Logística'::text, p_seed_tag text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  SELECT COALESCE(e.start_date, CURRENT_DATE) INTO v_stage_start
  FROM public.events e WHERE e.id = p_event_id;

  SELECT es.id INTO v_stage_id
  FROM public.event_stages es
  WHERE es.event_id = p_event_id AND es.slug = v_stage_slug
  LIMIT 1;
  ...
  IF v_stage_id IS NULL THEN
    ...
    VALUES (
      p_event_id, p_stage_name, v_stage_slug, 'regional', 'draft', 10,
      v_stage_start::timestamptz, (v_stage_start + INTERVAL '3 days')::timestamptz
    )
  ...
  SELECT id INTO v_loc_a FROM public.lodging_locations
  WHERE event_id = p_event_id AND seed_tag = v_seed_tag
  ORDER BY created_at ASC LIMIT 1;

  SELECT id INTO v_loc_b FROM public.lodging_locations
  WHERE event_id = p_event_id AND seed_tag = v_seed_tag
  ...
  INSERT INTO public.meal_types (event_id, event_stage_id, name, slug, sort_order, seed_tag, seed_batch_id)
  VALUES
  ...
  SELECT id INTO v_mt_cafe FROM public.meal_types WHERE event_id = p_event_id AND slug = 'cafe-da-manha' LIMIT 1;
  SELECT id INTO v_mt_almoco FROM public.meal_types WHERE event_id = p_event_id AND slug = 'almoco' LIMIT 1;
  SELECT id INTO v_mt_jantar FROM public.meal_types WHERE event_id = p_event_id AND slug = 'jantar' LIMIT 1;
  ...
  SELECT p_event_id, v_stage_id, v_mt_cafe, d::date, '06:30'::time, '08:30'::time, 'Refeitório Central', 'Café', v_seed_tag, v_seed_batch_id
  FROM generate_series(v_stage_start, v_stage_start + INTERVAL '2 days', INTERVAL '1 day') d
  UNION ALL
  SELECT p_event_id, v_stage_id, v_mt_almoco, d::date, '11:30'::time, '13:30'::time, 'Refeitório Central', 'Almoço', v_seed_tag, v_seed_batch_id
  FROM generate_series(v_stage_start, v_stage_start + INTERVAL '2 days', INTERVAL '1 day') d
  UNION ALL
  SELECT p_event_id, v_stage_id, v_mt_jantar, d::date, '18:00'::time, '20:00'::time, 'Refeitório Central', 'Jantar', v_seed_tag, v_seed_batch_id
  FROM generate_series(v_stage_start, v_stage_start + INTERVAL '2 days', INTERVAL '1 day') d;
  ...
  SELECT COUNT(*) INTO v_n FROM public.transport_trips
  WHERE event_id = p_event_id AND seed_tag = v_seed_tag;
  ...
  RETURN jsonb_build_object(
    ...
    'counts', jsonb_build_object(
      ...
      'meal_types', (SELECT COUNT(*) FROM public.meal_types WHERE event_id = p_event_id),
